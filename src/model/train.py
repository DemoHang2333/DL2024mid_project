...
from transformers import CLIPModel, CLIPProcessor, AutoProcessor, CLIPSegModel, VisualBertModel, AlignProcessor, AutoModelForCausalLM
...
try:
    optimizer = optim.SGD(model.parameters(), lr=0.005, momentum=0.9, weight_decay=1e-6, dampening=0, nesterov=True)   # pick one from torch.optim
...
def train(loger, train_dataloader, model, optimizer, device, processor):
    ...
    for idx, batch in progress_bar:
        ...
        text_features = outputs.text_embeds    # 文本特征

        similarity = torch.cosine_similarity(image_features, text_features, dim=1)
        loss = 1 - similarity.mean() #可以改
